...
class TestPeer:
    ...
    def control_loop(self, sock: socket.socket, remote_addr: Address) -> None:
        ...
        def receiver() -> None:
            logger.info(f"[C] Listening on {sock_formatted}")
            while True:
                _, addr = sock.recvfrom(1024)
                logger.info(f"[C] from {addr[0]}:{addr[1]}")

        def sender() -> None:
            logger.info(f"[C] Sending from {sock_formatted} to {remote_addr_formatted}")
            while True:
                sock.sendto(Heartbeat().to_bytes(), remote_addr)
                logger.info(f"[C] to   {remote_addr_formatted}")
                time.sleep(1)
