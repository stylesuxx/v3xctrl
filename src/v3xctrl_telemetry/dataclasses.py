...
class GstFlags:
    ...
    def from_byte(cls, byte: int) -> "GstFlags":
        ...
        return cls(recording=bool(byte & (1 << 0)), udp_overrun=bool(byte & (1 << 1)))
...
class ServiceFlags:
    ...
    def from_byte(cls, byte: int) -> "ServiceFlags":
        """Parse flags from byte value."""
        return cls(video=bool(byte & (1 << 0)), reverse_shell=bool(byte & (1 << 1)), debug=bool(byte & (1 << 2)))
...
class VideoCoreFlags:
    ...
    def from_byte(cls, byte: int) -> "VideoCoreFlags":
        """Parse flags from byte value."""
        return cls(
            current=ThrottleFlags.from_nibble(byte & 0x0F), history=ThrottleFlags.from_nibble((byte >> 4) & 0x0F)
        )
