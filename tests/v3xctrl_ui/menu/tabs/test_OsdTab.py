...
class TestOsdTab(unittest.TestCase):
    def setUp(self):
        ...
        self.tab = OsdTab(self.settings, width=640, height=480, padding=10, y_offset=0)
    ...
    def test_initial_checkbox_states(self):
        ...
        self.assertFalse(self.tab.checkboxes["battery_current"].checked)
    ...
    def test_checkbox_updates_settings(self):
        ...
        self.assertFalse(self.tab.widgets.get("debug").display)

        self.tab.checkboxes["steering"].set_checked(False)
        self.assertFalse(self.tab.widgets.get("steering").display)

        self.tab.checkboxes["throttle"].set_checked(True)
        self.assertTrue(self.tab.widgets.get("throttle").display)

        self.tab.checkboxes["battery_voltage"].set_checked(True)
        self.assertTrue(self.tab.widgets.get("battery_voltage").display)

        self.tab.checkboxes["battery_average_voltage"].set_checked(False)
        self.assertFalse(self.tab.widgets.get("battery_average_voltage").display)

        self.tab.checkboxes["battery_percent"].set_checked(True)
        self.assertTrue(self.tab.widgets.get("battery_percent").display)

        self.tab.checkboxes["battery_current"].set_checked(True)
        self.assertTrue(self.tab.widgets.get("battery_current").display)

        self.tab.checkboxes["signal_quality"].set_checked(False)
        self.assertFalse(self.tab.widgets.get("signal_quality").display)

        self.tab.checkboxes["signal_band"].set_checked(True)
        self.assertTrue(self.tab.widgets.get("signal_band").display)

        self.tab.checkboxes["signal_cell"].set_checked(False)
        self.assertFalse(self.tab.widgets.get("signal_cell").display)
    ...
    def test_get_settings_aggregation(self):
        ...
        self.assertFalse(settings["widgets"].get("debug").display)
        self.assertFalse(settings["widgets"].get("steering").display)
        self.assertTrue(settings["widgets"].get("battery_percent").display)
