...
class TestNetworkUnreachableSuppression(unittest.TestCase):
    ...
    def _make_udpsink_write_warning(self, mock_gst: MagicMock) -> MagicMock:
        message = MagicMock()
        message.type = mock_gst.MessageType.WARNING
        message.src.get_name.return_value = "udpsink"

        error = MagicMock()
        error.domain = mock_gst.resource_error_quark()
        error.code = mock_gst.ResourceError.WRITE
    ...
    def _make_other_warning(self, mock_gst: MagicMock, text: str) -> MagicMock:
        ...
        # Different domain so _is_udpsink_write_warning returns False
        error.domain = "other-domain"
        error.code = -1
    ...
    def test_first_unreachable_warning_is_logged(self, mock_gst: MagicMock, mock_cs: MagicMock):
        ...
        message = self._make_udpsink_write_warning(mock_gst)
