...
class TestBuildSeiNal(unittest.TestCase):
    ...
    def test_contains_timestamp_and_offset(self):
        sei = build_sei_nal(123456789, -500)

        timestamp, offset = struct.unpack(">qq", sei[23:39])
    ...
    def test_total_length(self):
        ...
        # 4 (start code) + 1 (NAL header) + 1 (type) + 1 (size) + 16 (UUID) + 16 (data) + 1 (RBSP) = 40
        self.assertEqual(len(sei), 40)

    def test_negative_timestamp(self):
        sei = build_sei_nal(-1000000, 200)
...
class TestParseSeiNal(unittest.TestCase):
    def test_roundtrip(self):
        sei = build_sei_nal(123456789, -42)

        result = parse_sei_nal(sei)

        self.assertIsNotNone(result)
        self.assertEqual(result, (123456789, -42))

    def test_sei_prepended_to_frame_data(self):
        sei = build_sei_nal(999, 111)
        ...
        self.assertIsNotNone(result)
        self.assertEqual(result, (999, 111))
    ...
    def test_large_values(self):
        ...
        sei = build_sei_nal(ts, offset)
        result = parse_sei_nal(sei)

        self.assertEqual(result, (ts, offset))
